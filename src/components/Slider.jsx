    ...
    <>
      {modalState &&
        <div className=' w-screen h-screen fixed top-0 left-0 bg-black bg-opacity-90 flex items-center justify-center z-50'>
          <div className=' w-[86vh] shadow-xl mx-auto flex items-center justify-center md:w-[82.3vw] md:h-[84vh]'>
            ...
            <div ref={carrusel} className=' flex gap-4 p-4 w-full snap-mandatory overflow-x-auto scroll-p-3 absolute bottom-0 scroll-smooth'>
              {
                images.map((image, index) => {
                  return (
                    <figure onClick={() => handleClick(index)} className=' w-28 h-24 rounded overflow-hidden shadow-lg shrink-0 snap-center cursor-pointer md:w-44 md:h-32 lg:w-96 lg:h-80' key={index}>
